  ...
  return (
    <div className="relative w-full">
      <div className="inline-block text-left w-full">
        <button
          onClick={toggleDropdown}
          className="inline-flex justify-between items-center w-full sm:w-56 px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none"
        >
          Dịch vụ đi kèm
          {isOpen ? (
            <IoChevronUp className="w-5 h-5 ml-2" />
        ...
        {isOpen && (
          <div className="w-full sm:w-56 mt-2 bg-white border border-gray-200 rounded-md shadow-lg">
            <div className="p-4">
              <label className="flex items-center mb-2 text-xs sm:text-sm">
                <input
                  type="checkbox"
                  className="mr-2"
      ...
      {isLetterChecked && (
        <div className="mt-4">
          <input
            className="w-full sm:w-[450px] h-[40px]   sm:h-[63px] p-2 bg-white border border-gray-200 rounded-xl text-black shadow-lg"
