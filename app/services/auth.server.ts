...
authenticator.use(
  new FormStrategy(async ({ form }) => {
    let { email, password, loginType } = await parseFormData<LoginFormData>(form);
    console.log('loginType', loginType);
    
    try {
      if (loginType == 'user-pass') {
        const result = await axios.post(`${BASE_URL}/api/Account/Login`, { email, password: password.toString() }, {
          headers: {
